...
def do_train(checkpoint=None, direction='amr', split_both_decoder=False, fp16=False):
    ...
    visibility_rate_dict = {VisibilityMatrixConstructor.SAME_CLAUSE: config[VisibilityMatrixConstructor.SAME_CLAUSE],
                            VisibilityMatrixConstructor.ADJACENT_CLAUSE: config[VisibilityMatrixConstructor.ADJACENT_CLAUSE],
                            VisibilityMatrixConstructor.NONADJACENT_CLAUSE: config[VisibilityMatrixConstructor.NONADJACENT_CLAUSE],
                            VisibilityMatrixConstructor.KEYWORD_TOKEN: config[VisibilityMatrixConstructor.KEYWORD_TOKEN],
                            VisibilityMatrixConstructor.REENTRANT_TOKEN: config[VisibilityMatrixConstructor.REENTRANT_TOKEN],
                            }
    ...
    model, tokenizer = instantiate_model_and_tokenizer(
        ...
        clause_token_visibility=config.get('clause_token_visibility', False),
        clause_token_inf_mask=config.get('clause_token_inf_mask', False),
        visibility_rate_dict=visibility_rate_dict,
        clause_attn_head_num=config.get('clause_attn_head_num', 0),
        clause_attn_layer_id=config.get('clause_attn_layer_id', None),
    )
    ...
    train_loader = instantiate_loader(
        ...
        input_features_path=config['train_features'],
    )
    ...
    dev_loader = instantiate_loader(
        ...
        remove_wiki=config['remove_wiki'],
        dereify=config['dereify'],
        input_features_path=config['dev_features'],
    )
    ...
    if direction == 'amr':
        ...
        def train_step(engine, batch):
            model.train()
            x, y, extra = batch
            # for eg in extra['graphs']:
            #     if eg.metadata['id'] == 'DF-199-192823-675_9881.2':
            #         print()
    ...
    if config['save_checkpoints']:
        ...
        if config['log_wandb']:
            ...
            where_checkpoints = root/config['pt_folder_name']
            try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--direction', type=str, default='amr', choices=['amr', 'text', 'both'], help='Train a uni- (amr, text) or bidirectional (both).')
    parser.add_argument('--split-both-decoder', action='store_true')
    parser.add_argument('--config', type=Path, default=ROOT/'configs/sweeped.yaml', help='Use the following config for hparams.')
