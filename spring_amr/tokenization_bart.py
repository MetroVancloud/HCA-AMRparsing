...
class AMRBartTokenizer(BartTokenizer):
    ...
    def _tokenize(self, text, features_list=None):
        ...
        clause_id = 0
        current_clause_id = -1
        clause_index_to_clause_id_dict = dict()
        clause_id_stack = []
        for tok_span in features_list['dfs_clauses']:
            if tok_span in VisibilityMatrixConstructor.CLAUSE_TAG:
                ...
                if current_clause_id == -1:
                    print('ss')
                clause_index_to_clause_id_dict[tok_span] = current_clause_id

        for tok_span in text.lstrip().split(' '):
            try:
                token_pos = features_list['pos'][token_id]
                token_ner = features_list['ner'][token_id]
                token_clause_index = features_list['index'][token_id]
                token_id += 1
            except:
                print('error')
            ...
            for sub_t in new_added_sub_tokens:
                sub_token_clause_index_list.append(token_clause_index)
                sub_token_pos_list.append(token_pos)
                sub_token_ner_list.append(token_ner)
        
        token_visibility_matrix = VisibilityMatrixConstructor.construct_visibility_matrix2(clause_adjacent_matrix=clause_adjacency_matrix, sub_token_clause_index_list=sub_token_clause_index_list, visibility_rate_dict=self.visibility_rate_dict, sub_token_pos_list=sub_token_pos_list, sub_token_ner_list=sub_token_ner_list, clause_index_to_clause_id_dict=clause_index_to_clause_id_dict, clause_token_inf_mask=self.clause_token_inf_mask)
    ...
    def batch_encode_sentences(self, sentences, features_list=None, device=torch.device('cpu')):
    ...
    def batch_encode_plus(
        ...
        for first_ids, second_ids, token_visibility_matrix in input_ids:
            ...
            # Fanyunlong
            modified_token_visibility_matrix = VisibilityMatrixConstructor.reconstruct_visibility_matrix_by_max_length(token_visibility_matrix, max_length, self.clause_token_inf_mask)
