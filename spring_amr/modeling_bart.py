...
import spring_amr.transformers_modeling_bart as bart
...
class AMRBartEncoder(nn.Module):
    ...
    def __init__(self, config: bart.BartConfig, embed_tokens, backpointer_idx, clause_attn_head_num=0, clause_attn_layer_id={}, clause_token_inf_mask=False):
        super().__init__()
        ...
        # Fanyunlong hyper params dilivered
        self.clause_attn_head_num = clause_attn_head_num
        self.clause_attn_layer_id = clause_attn_layer_id
        self.clause_token_inf_mask = clause_token_inf_mask

    def forward(
        self, input_ids, embedded=None, attention_mask=None, token_visibility_matrix=None
    ):
        """
        Args:
        ...
        # B x T x C -> T x B x C
        # Shape: Batch_size * Sequence_length * dim_model(word_embed_size)
        # ->
        # Shape: Sequence_length * Batch_size * dim_model(word_embed_size)
        x = x.transpose(0, 1)
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.training and (dropout_probability < self.layerdrop):  # skip the layer
                ...
                # attention_mask: mask padding tokens
                # shape: Batch_size * Sequence_length
                if str(idx) in self.clause_attn_layer_id:
                    x, attn = encoder_layer(x, attention_mask, token_visibility_matrix, self.clause_attn_head_num, self.clause_token_inf_mask)
                else:
...
class AMRBartModel(bart.PretrainedBartModel):
    def __init__(self, config: bart.BartConfig, backpointer_idx=None, clause_attn_head_num=0, clause_attn_layer_id={}, clause_token_inf_mask=False):
        ...
        self.encoder = AMRBartEncoder(config, self.shared, backpointer_idx=self.backpointer_idx, clause_attn_head_num=clause_attn_head_num, clause_attn_layer_id=clause_attn_layer_id, clause_token_inf_mask=clause_token_inf_mask)
    ...
    def forward(
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(input_ids=input_ids, token_visibility_matrix=token_visibility_matrix, attention_mask=attention_mask)
...
class AMRBartForConditionalGeneration(bart.PretrainedBartModel):
    ...
    def __init__(self, config: bart.BartConfig, backpointer_idx=None, clause_token_inf_mask=False, clause_attn_head_num=0, clause_attn_layer_id={}):
        super().__init__(config)
        base_model = AMRBartModel(config, backpointer_idx, clause_attn_head_num, clause_attn_layer_id, clause_token_inf_mask=clause_token_inf_mask)
    ...
    Examples::
        ...
        outputs = self.compute_logits(
            ...
            decoder_cached_states=decoder_cached_states,
            use_cache=use_cache,
            token_visibility_matrix=token_visibility_matrix,
        )
    ...
    def generate(
        ...
        if self.config.is_encoder_decoder:
            ...
            encoder_outputs: tuple = encoder(input_ids, attention_mask=attention_mask, token_visibility_matrix=model_specific_kwargs['token_visibility_matrix'])
